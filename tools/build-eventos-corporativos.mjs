...
.carrossel__contagem {
  ...
  /* --cor-label (#AE9C72) dá 2,3:1 sobre o creme da galeria; este tom dá 4,5:1 */
  color: #7D6A3F;
}
...
.palco__atalho {
  ...
  color: var(--cor-texto);
  /* .5 de opacidade dava 2,8:1 sobre o fundo claro — abaixo do mínimo.
     .72 mantém a hierarquia com o item ativo e chega a 4,9:1. */
  opacity: .72;
  padding: 10px 12px;
  min-height: 40px;
  border-radius: 999px;
...
/* No celular só a etapa 2 emparelha (data+convidados): são campos curtos e o
   par economiza uma linha. A etapa 1 fica com um campo por linha — nome de
   empresa e e-mail precisam da largura inteira para serem digitados sem
   apertar. O align-items: end alinha os campos quando um rótulo quebra em
   duas linhas. */
@media (min-width: 360px) and (max-width: 639px) {
  .form-etapa[data-etapa="2"] .campos-duplos {
    grid-template-columns: 1fr 1fr;
    gap: 0 10px;
    align-items: end;
  }
  /* meia largura: o respiro lateral tem que ceder para o texto caber */
  .form-etapa[data-etapa="2"] .campos-duplos .campo input,
  .form-etapa[data-etapa="2"] .campos-duplos .campo select { padding-inline: 11px; }
}
...
@media (max-width: 899px) {
  ...
  .formulario--etapas .formulario__passo {
    ...
    /* --cor-label (#AE9C72) sobre branco dá 2,7:1 — abaixo do mínimo.
       Este tom da mesma família dá 5,2:1. */
    color: #7D6A3F;
  }
...
/* O CSS do design declara a posição mobile do pontinho dentro do
   @media (max-width: 899px), mas a regra base ".hospedagem__destaque::before"
   aparece DEPOIS no arquivo, com a mesma especificidade — e vence. Resultado:
   no celular o ponto ia para o canto superior esquerdo da célula, solto do
   texto centralizado. Redeclarado aqui, que é o fim da folha. */
@media (max-width: 899px) {
  .hospedagem__destaque::before { left: 50%; top: 0; transform: translateX(-50%); }
}
